...
//     /// Returns [`AllDependents`], an [`Iterator`] over [`Schema`]s which
...
//     /// Starts a new transaction with `schemas` and `sources`
...
//     fn commit_txn<T, E>(&mut self, val: T) -> Result<T, E> {
...
//     fn rollback_txn<T, E>(&mut self, err: E) -> Result<T, E> {
...
// pub struct Build<L, K> where L: Language<K>, K: 'static + Key {
