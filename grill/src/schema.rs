...
impl<'i, K: slotmap::Key> PartialEq for Schema<'i, K> {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id && self.metaschema == other.metaschema
    }
}

impl<'i, K: slotmap::Key> Eq for Schema<'i, K> {}
...
impl<Key: slotmap::Key> DependencyGraph<Key> {
    /// Creates a new [`DependencyGraph<Key>`].
    #[must_use]
    pub fn new() -> Self {
        Self {
...
impl<Key: slotmap::Key> Default for DependencyGraph<Key> {
    fn default() -> Self {
        Self::new()
    }
}
...
/// A newtype used to distinguish between keywords and other strings as
...
impl Keyword<'_> {
    ...
    pub const ID_LEGACY: Keyword<'static> = Keyword("id");
