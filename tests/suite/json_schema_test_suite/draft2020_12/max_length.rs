...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_max_length(interrogator)
    }
    interrogator
}
mod max_length_validation_0 {
    ...
    const URI: &str = "http://localhost:1234/maxLength.json";
    const DESCRIPTION: &str = "maxLength validation";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/maxLength.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
    ...
    fn test4_two_supplementary_unicode_code_points_is_long_enough() {
        let description = "two supplementary Unicode code points is long enough";
        let data = "\"\\uD83D\\uDCA9\\uD83D\\uDCA9\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
...
mod max_length_validation_with_a_decimal_1 {
    ...
    const URI: &str = "http://localhost:1234/maxLength.json";
    const DESCRIPTION: &str = "maxLength validation with a decimal";
    fn setup() -> Result<(Key, Interrogator), &'static CompileError> {
        use std::sync::OnceLock;
        static INTERROGATOR: OnceLock<Result<(Key, Interrogator), CompileError>> = OnceLock::new();
        INTERROGATOR
