...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_optional_format_unknown(interrogator)
    }
    interrogator
}
mod unknown_format_0 {
    ...
    const URI: &str = "http://localhost:1234/unknown.json";
    const DESCRIPTION: &str = "unknown format";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/unknown.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
