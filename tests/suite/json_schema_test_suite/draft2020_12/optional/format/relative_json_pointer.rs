...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_optional_format_relative_json_pointer(interrogator)
    }
    interrogator
}
mod validation_of_relative_json_pointers_rjp_0 {
    ...
    const URI: &str = "http://localhost:1234/relative-json-pointer.json";
    const DESCRIPTION: &str = "validation of Relative JSON Pointers (RJP)";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(
                    interrogator.compile("http://localhost:1234/relative-json-pointer.json"),
                )?;
                Ok((key, interrogator))
            })
            .as_ref()
