...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_optional_format_iri_reference(interrogator)
    }
    interrogator
}
mod validation_of_iri_references_0 {
    ...
    const URI: &str = "http://localhost:1234/iri-reference.json";
    const DESCRIPTION: &str = "validation of IRI References";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key =
                    block_on(interrogator.compile("http://localhost:1234/iri-reference.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
    ...
    fn test7_a_valid_protocol_relative_iri_reference() {
        let description = "a valid protocol-relative IRI Reference";
        let data = "\"//ƒøø.ßår/?∂éœ=πîx#πîüx\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test8_a_valid_relative_iri_reference() {
        let description = "a valid relative IRI Reference";
        let data = "\"/âππ\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test9_an_invalid_iri_reference() {
        let description = "an invalid IRI Reference";
    ...
    fn test10_a_valid_iri_reference() {
        let description = "a valid IRI Reference";
        let data = "\"âππ\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test11_a_valid_iri_fragment() {
        let description = "a valid IRI fragment";
        let data = "\"#ƒrägmênt\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test12_an_invalid_iri_fragment() {
        let description = "an invalid IRI fragment";
        let data = "\"#ƒräg\\\\mênt\"";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
