...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_optional_format_uri(interrogator)
    }
    interrogator
}
mod validation_of_ur_is_0 {
    ...
    const URI: &str = "http://localhost:1234/uri.json";
    const DESCRIPTION: &str = "validation of URIs";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/uri.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
    ...
    fn test6_a_valid_url_with_anchor_tag() {
        let description = "a valid URL with anchor tag";
        let data = "\"http://foo.bar/?baz=qux#quux\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test7_a_valid_url_with_anchor_tag_and_parentheses() {
        let description = "a valid URL with anchor tag and parentheses";
        let data = "\"http://foo.com/blah_(wikipedia)_blah#cite-1\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test8_a_valid_url_with_url_encoded_stuff() {
        let description = "a valid URL with URL-encoded stuff";
        let data = "\"http://foo.bar/?q=Test%20URL-encoded%20stuff\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test9_a_valid_puny_coded_url() {
        let description = "a valid puny-coded URL ";
        let data = "\"http://xn--nw2a.xn--j6w193g/\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test10_a_valid_url_with_many_special_characters() {
        let description = "a valid URL with many special characters";
    ...
    fn test11_a_valid_url_based_on_i_pv4() {
        let description = "a valid URL based on IPv4";
        let data = "\"http://223.255.255.254\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test12_a_valid_url_with_ftp_scheme() {
        let description = "a valid URL with ftp scheme";
        let data = "\"ftp://ftp.is.co.za/rfc/rfc1808.txt\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test13_a_valid_url_for_a_simple_text_file() {
        let description = "a valid URL for a simple text file";
        let data = "\"http://www.ietf.org/rfc/rfc2396.txt\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test14_a_valid_url() {
        let description = "a valid URL ";
        let data = "\"ldap://[2001:db8::7]/c=GB?objectClass?one\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test15_a_valid_mailto_uri() {
        let description = "a valid mailto URI";
        let data = "\"mailto:John.Doe@example.com\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test16_a_valid_newsgroup_uri() {
        let description = "a valid newsgroup URI";
        let data = "\"news:comp.infosystems.www.servers.unix\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test17_a_valid_tel_uri() {
        let description = "a valid tel URI";
        let data = "\"tel:+1-816-555-1212\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test18_a_valid_urn() {
        let description = "a valid URN";
        let data = "\"urn:oasis:names:specification:docbook:dtd:xml:4.1.2\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test19_an_invalid_protocol_relative_uri_reference() {
        let description = "an invalid protocol-relative URI Reference";
    ...
    fn test20_an_invalid_relative_uri_reference() {
        let description = "an invalid relative URI Reference";
        let data = "\"/abc\"";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test21_an_invalid_uri() {
        let description = "an invalid URI";
    ...
    fn test22_an_invalid_uri_though_valid_uri_reference() {
        let description = "an invalid URI though valid URI reference";
        let data = "\"abc\"";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test23_an_invalid_uri_with_spaces() {
        let description = "an invalid URI with spaces";
        let data = "\"http:// shouldfail.com\"";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test24_an_invalid_uri_with_spaces_and_missing_scheme() {
        let description = "an invalid URI with spaces and missing scheme";
        let data = "\":// should fail\"";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
    ...
    fn test25_an_invalid_uri_with_comma_in_scheme() {
        let description = "an invalid URI with comma in scheme";
        let data = "\"bar,baz:foo\"";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
            Err(err) => {
