...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_optional_format_idn_email(interrogator)
    }
    interrogator
}
mod validation_of_an_internationalized_e_mail_addresses_0 {
    ...
    const URI: &str = "http://localhost:1234/idn-email.json";
    const DESCRIPTION: &str = "validation of an internationalized e-mail addresses";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/idn-email.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
    ...
    fn test6_a_valid_idn_e_mail_example_example_test_in_hangul() {
        let description = "a valid idn e-mail (example@example.test in Hangul)";
        let data = "\"실례@실례.테스트\"";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test7_an_invalid_idn_e_mail_address() {
        let description = "an invalid idn e-mail address";
        let data = "\"2962\"";
