...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_defs(interrogator)
    }
    interrogator
}
mod validate_definition_against_metaschema_0 {
    ...
    const URI: &str = "http://localhost:1234/defs.json";
    const DESCRIPTION: &str = "validate definition against metaschema";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/defs.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
