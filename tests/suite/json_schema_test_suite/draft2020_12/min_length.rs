...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_min_length(interrogator)
    }
    interrogator
}
mod min_length_validation_0 {
    ...
    const URI: &str = "http://localhost:1234/minLength.json";
    const DESCRIPTION: &str = "minLength validation";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/minLength.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
    ...
    fn test4_one_supplementary_unicode_code_point_is_not_long_enough() {
        let description = "one supplementary Unicode code point is not long enough";
        let data = "\"\\uD83D\\uDCA9\"";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
...
mod min_length_validation_with_a_decimal_1 {
    ...
    const URI: &str = "http://localhost:1234/minLength.json";
    const DESCRIPTION: &str = "minLength validation with a decimal";
    fn setup() -> Result<(Key, Interrogator), &'static CompileError> {
        use std::sync::OnceLock;
        static INTERROGATOR: OnceLock<Result<(Key, Interrogator), CompileError>> = OnceLock::new();
        INTERROGATOR
