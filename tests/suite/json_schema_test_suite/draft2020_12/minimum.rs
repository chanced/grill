...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_minimum(interrogator)
    }
    interrogator
}
mod minimum_validation_0 {
    ...
    const URI: &str = "http://localhost:1234/minimum.json";
    const DESCRIPTION: &str = "minimum validation";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/minimum.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
    ...
    fn test0_above_the_minimum_is_valid() {
        let description = "above the minimum is valid";
        let data = "2.6";
    ...
    fn test2_below_the_minimum_is_invalid() {
        let description = "below the minimum is invalid";
        let data = "0.6";
...
mod minimum_validation_with_signed_integer_1 {
    ...
    const URI: &str = "http://localhost:1234/minimum.json";
    const DESCRIPTION: &str = "minimum validation with signed integer";
    fn setup() -> Result<(Key, Interrogator), &'static CompileError> {
        use std::sync::OnceLock;
        static INTERROGATOR: OnceLock<Result<(Key, Interrogator), CompileError>> = OnceLock::new();
        INTERROGATOR
    ...
    fn test0_negative_above_the_minimum_is_valid() {
        let description = "negative above the minimum is valid";
        let data = "-1";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test1_positive_above_the_minimum_is_valid() {
        let description = "positive above the minimum is valid";
        let data = "0";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test3_boundary_point_with_float_is_valid() {
        let description = "boundary point with float is valid";
        let data = "-2.0";
        let expected_valid = true;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test4_float_below_the_minimum_is_invalid() {
        let description = "float below the minimum is invalid";
        let data = "-2.0001";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
    ...
    fn test5_int_below_the_minimum_is_invalid() {
        let description = "int below the minimum is invalid";
        let data = "-3";
        let expected_valid = false;
        let (key, interrogator) = match setup() {
            Ok((key, interrogator)) => (key, interrogator),
