...
fn interrogator() -> Result<Interrogator, &'static BuildError> {
    ...
    if let Ok(interrogator) = interrogator.as_mut() {
        crate::Harness.setup_max_items(interrogator)
    }
    interrogator
}
mod max_items_validation_0 {
    ...
    const URI: &str = "http://localhost:1234/maxItems.json";
    const DESCRIPTION: &str = "maxItems validation";
        ...
        INTERROGATOR
            .get_or_init(|| {
                ...
                interrogator
                    ...
                    .unwrap();
                let key = block_on(interrogator.compile("http://localhost:1234/maxItems.json"))?;
                Ok((key, interrogator))
            })
            .as_ref()
            .map(Clone::clone)
...
mod max_items_validation_with_a_decimal_1 {
    ...
    const URI: &str = "http://localhost:1234/maxItems.json";
    const DESCRIPTION: &str = "maxItems validation with a decimal";
    fn setup() -> Result<(Key, Interrogator), &'static CompileError> {
        use std::sync::OnceLock;
        static INTERROGATOR: OnceLock<Result<(Key, Interrogator), CompileError>> = OnceLock::new();
        INTERROGATOR
